""" Training script for the face completion model. """
...
def main(exp_dir, train_dir, val_dir=None, workers=4, iterations=None, epochs=(90,), start_epoch=None,
         ...
         gen_inpainter='res_unet_split.MultiScaleResUNet(in_nc=4,out_nc=(3,3))',
         discriminator='discriminators_pix2pix.MultiscaleDiscriminator',
         rec_weight=0.5, gan_weight=0.1, background_value=-1.0):
    ...
    Gi = obj_factory(gen_inpainter).to(device)
    ...
    Gi_path = os.path.join(checkpoint_dir, 'Gi_latest.pth')
    ...
    if os.path.isfile(Gr_path):
        ...
        # Gi
        Gi.apply(utils.init_weights)
        if os.path.isfile(Gi_path):
            checkpoint = torch.load(Gi_path)
            ...
            Gi.load_state_dict(checkpoint['state_dict'], strict=False)
    ...
    for ri in range(start_res_ind, len(resolutions)):
        ...
        optimizer_G = obj_factory(optimizer, Gi.parameters(), lr=res_lr_gen)
        ...
        for epoch in range(start_epoch, res_epochs):
            ...
            for i, (img1, landmarks1, bbox1, seg1, target1, img2, landmarks2, bbox2, seg2, target2) in enumerate(pbar):
                ...
                # Source face mask
                seg_pred_face_mask = seg_pred[0].argmax(1) == 1
                inpainting_mask = seg_utils.random_hair_inpainting_mask_tensor(seg_pred_face_mask).to(device)
                source_face_mask = seg_pred_face_mask * (inpainting_mask == 0)
                source_face_mask = ~source_face_mask
                # source_face_mask = source_face_mask.unsqueeze(1).repeat(1, 3, 1, 1).float()
                img_pred_with_hole = img_pred[0].clone()
                img_pred_with_hole.masked_fill_(source_face_mask.unsqueeze(1), background_value)

                # Target face
                target_face_mask = seg2[0] != 1
                inpainting_target = img2[0]
                inpainting_target.masked_fill_(target_face_mask.unsqueeze(1), background_value)

                # Face inpainting
                face_mask = (seg2[0] == 1).unsqueeze(1).float()
                inpainting_input = torch.cat((img_pred_with_hole, face_mask), dim=1)
                inpainting_input_pyd = img_utils.create_pyramid(inpainting_input, len(img2))
                inpainting_pred = Gi(inpainting_input_pyd)

                # Fake Detection and Loss
                img_inpainting_pred_pyd = img_utils.create_pyramid(inpainting_pred, len(img1))
                pred_fake_pool = D([x.detach() for x in img_inpainting_pred_pyd])
                loss_D_fake = criterion_gan(pred_fake_pool, False)

                # Real Detection and Loss
                inpainting_target_pyd = img_utils.create_pyramid(inpainting_target, len(img1))
                ...
                pred_fake = D(img_inpainting_pred_pyd)
            ...
            with torch.no_grad():
                ...
                for i, (img1, landmarks1, bbox1, seg1, target1, img2, landmarks2, bbox2, seg2, target2) in enumerate(pbar):
                    ...
                    face_mask = (seg2[0] == 1).unsqueeze(1).float()
                    inpainting_input = torch.cat((img_pred_with_hole, face_mask), dim=1)
                    inpainting_input_pyd = utils.create_pyramid(inpainting_input, len(img2))
                    inpainting_pred = Gi(inpainting_input_pyd)

                    # Fake Detection and Loss
                    img_inpainting_pred_pyd = utils.create_pyramid(inpainting_pred, len(img1))
                    pred_fake_pool = D([x.detach() for x in img_inpainting_pred_pyd])
                    loss_D_fake = criterion_gan(pred_fake_pool, False)

                    # Real Detection and Loss
                    inpainting_target_pyd = utils.create_pyramid(inpainting_target, len(img1))
                    pred_real = D(inpainting_target_pyd)
                    loss_D_real = criterion_gan(pred_real, True)
                ...
                grid = img_utils.make_grid(img1[0], img_pred[0], img_pred_with_hole, inpainting_pred, inpainting_target)
            ...
            utils.save_checkpoint(exp_dir, 'Gi', {
                'resolution': res,
                'epoch': epoch + 1,
                'state_dict': Gi.module.state_dict() if gpus and len(gpus) > 1 else Gi.state_dict(),
...
#     if len(a[0]) == 0 or len(a[1]) == 0:
#         return target_img
#     if (np.max(a[0]) - np.min(a[0])) <= 2 or (np.max(a[1]) - np.min(a[1])) <= 2:
#         return target_img
#
#     center = (np.min(a[0]) + np.max(a[0])) // 2, (np.min(a[1]) + np.max(a[1])) // 2
#     if (np.max(a[0]) - np.min(a[0])) <= 4 or (np.max(a[1]) - np.min(a[1])) <= 4:
#         print('source_img.shape = ' + str(source_img.shape))
#         print('target_img.shape = ' + str(target_img.shape))
#         print('mask.shape = ' + str(mask.shape))
#         print('center = ' + str(center))
#         print('x range = (%d, %d)' % (np.min(a[0]), np.max(a[0])))
#         print('y range = (%d, %d)' % (np.min(a[1]), np.max(a[1])))
#
#     try:
#         output = cv2.seamlessClone(source_img, target_img, mask*255, center, cv2.NORMAL_CLONE)
#     except:
...
# def make_grid(input, pred, transfer, blend, swap, gt, cols=8):
#     imgs = []
...
if __name__ == "__main__":
    ...
    import argparse
    parser = argparse.ArgumentParser('Train inpainting (GAN)')
    ...
    parser.add_argument('-gi', '--gen_inpainter', default='res_unet_split.MultiScaleResUNet(in_nc=4,out_nc=(3,3))',
                        help='generator inpainter object')
    parser.add_argument('-d', '--discriminator', default='discriminators_pix2pix.MultiscaleDiscriminator',
                        help='discriminator object')
    parser.add_argument('-rw', '--rec_weight', default=0.5, type=float, metavar='F',
    ...
    main(args.exp_dir, args.train, args.val, workers=args.workers, iterations=args.iterations, epochs=args.epochs,
         ...
         gen_renderer=args.gen_renderer, gen_inpainter=args.gen_inpainter, discriminator=args.discriminator,
         rec_weight=args.rec_weight, gan_weight=args.gan_weight, background_value=args.background_value)
