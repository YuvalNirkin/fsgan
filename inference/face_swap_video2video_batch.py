...
def main(input, out_dir,
    ...
    if len(input) == 1:
        ...
        elif os.path.isfile(input[0]) and os.path.isdir(input[1]):
            src_paths = [input[0]]
            tgt_paths = glob(os.path.join(input[1], '*.mp4'))
            pair_paths = list(product(src_paths, tgt_paths))
            pair_paths = [(p1, p2) for p1, p2 in pair_paths if os.path.basename(p1) != os.path.basename(p2)]
    ...
    for i, (src_vid_path, tgt_vid_path) in enumerate(pair_paths):
        ...
        if os.path.exists(out_vid_path):
            ...
            try:
                face_swap_video2video.main(src_vid_path, tgt_vid_path, arch, reenactment_model_path, seg_model_path,
                                           inpainting_model_path, blend_model_path, pose_model_path, pil_transforms1,
                                           pil_transforms2, tensor_transforms1, tensor_transforms2, out_dir, min_radius,
                                           crop_size, reverse_output, verbose, output_crop, display)
...
if __name__ == "__main__":
    ...
    import argparse
    parser = argparse.ArgumentParser('face_swap_video2video_batch')
    ...
    main(args.input, args.output, args.arch, args.reenactment_model, args.seg_model, args.inpainting_model,
         ...
         args.tensor_transforms2, args.min_radius, args.crop_size, args.reverse_output, args.verbose, args.output_crop,
         args.display)
