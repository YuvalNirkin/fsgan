...
def main(source_path, target_path, frontal_path, arch, model_path, pil_transforms1=None, pil_transforms2=None,
    ...
    for i in range(len(target_landmarks)):
        target_landmarks[i] = target_landmarks[i].unsqueeze(0).to(device)
    ...
    # Save fronatal landmarks
    # landmarks = np.array([x.numpy() for x in frontal_landmarks])
    # np.save('frontal_landmarks_256_2_0.npy', landmarks)
    ...
    # euler3 = estimate_pose.matrix2angle(R3)  # Yaw, Pitch, Roll
    ...
    reenactment_img_pyd_tensor = []
    for j in range(len(source_tensor)):
        reenactment_img_pyd_tensor.append(source_tensor[j].unsqueeze(0))
    ...
    for alpha in np.arange(0.0, 1.0, 0.01):
        ...
        #
        # # Generate heatmaps
        # out_pts_tensor = torch.from_numpy(out_pts)
        # out_hm_tensor = landmarks2heatmaps(out_pts_tensor.unsqueeze(0).to(device))
        # out_hm_pyd_tensors = utils.create_pyramid(out_hm_tensor, len(source_tensor))

        # Prepare reenactment input
        reenactment_input = []
        for j in range(len(source_tensor)):
            reenactment_input.append(torch.cat((reenactment_img_pyd_tensor[j], target_landmarks[j]), dim=1))

        # Run face reenactment model
        reenactment_img_tensor, reenactment_seg_tensor = G(reenactment_input)
        reenactment_img_pyd_tensor = utils.create_pyramid(reenactment_img_tensor, len(source_tensor))
        reenactment_bgr = utils.tensor2bgr(reenactment_img_tensor)

        # Render landmarks
        source_render = plot_kpt(source_cropped_bgr, source_landmarks[0].numpy())
        # interp_render = plot_kpt(reenactment_bgr, out_pts)
        interp_render = reenactment_bgr
        # cv2.imshow('render', source_render)
        target_render = plot_kpt(target_cropped_bgr, source_landmarks[0].numpy())
