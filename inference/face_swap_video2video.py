...
def main(source_path, target_path,
    ...
    target_vid_width = int(target_vid.get(cv2.CAP_PROP_FRAME_WIDTH))
    target_vid_height = int(target_vid.get(cv2.CAP_PROP_FRAME_HEIGHT))
    ...
    if output_path is not None:
        ...
        if verbose == 1:
            ...
            out_vid_size = (crop_size * 3, crop_size * 3)
        else:   # verbose == 0
            if output_crop:
    ...
    for i in tqdm(range(total_frames)):
        ...
        for j in range(len(frame_landmarks)):
            curr_source_batch_tensors = torch.cat([p[j].unsqueeze(0) for p in source_pyd_batch_tensors], dim=0).to(
                device)
            curr_heatmaps = frame_landmarks[j].unsqueeze(0).repeat(curr_source_batch_tensors.shape[0], 1, 1, 1).to(
                device)
        ...
        inpainting_input_tensor_pyd = create_pyramid(inpainting_input_tensor, len(frame_tensor))
        ...
        blend_input_tensor_pyd = create_pyramid(blend_input_tensor, len(frame_tensor))
        ...
        if verbose == 0:    # Complete pipeline
            ...
            pose_axis_bgr = draw_axis(np.zeros_like(frame_cropped_bgr), frame_euler[0], frame_euler[1], frame_euler[2])
            ...
            render_img3 = np.concatenate((pose_axis_bgr, blend_img, frame_cropped_bgr), axis=1)
...
if __name__ == "__main__":
    ...
    import argparse
    parser = argparse.ArgumentParser('face_swap_video2video',
                                     description='''Video to video face swapping. An appearance map for the source 
    ...
    parser.add_argument('-v', '--verbose', default=0, type=int, metavar='N',
                        help='extra information level. 0 for rendered image only, 1 for ablation study.')
